  ...
  switch (parser.command_letter) {
    ...
    case 'M': switch (parser.codenum) {
      ...
      case 322: M322(); break; // управление пылесосом (мосфет на контакте HEATER_BED_PIN )
        ...
        case 329: GcodeSuite::M329(); break;
      #endif
...
//Самодельный зонд
  ...
  // функция для второго касания
    ...
    float second_probe(float measured_z) {
      ...
      float target_retract_z = measured_z + (float) PROBE_DIST_SECOND_RETRACT;
      feedrate_mm_s = (float) PROBE_SPEED_RETRACT;
      destination = current_position;
      destination.z = target_retract_z;
      prepare_line_to_destination();
      planner.synchronize();

      //опускание второй раз
      float target_second_z = measured_z - (float) PROBE_DIST_SECOND;
      feedrate_mm_s = (float) PROBE_SECOND_SPEED;
      destination = current_position;
  ...
  bool first_brobe() {
    ...
    bool probe_triggered = false; // сработал ли датчик
    float measured_z = 0; // главная переменная измеренной высоты
    float target_z = start_z - (float) PROBE_DIST;
    feedrate_mm_s = (float) PROBE_SPEED;
    ...
    if (probe_triggered) {
      SERIAL_ECHOLNPAIR_F("Triggered: ", measured_z, 4);
      ...
      SERIAL_ECHOLNPGM("ERROR: Probe not triggered");
